...
from armpi_fpv_kinematics.forward_kinematics import ForwardKinematics
from armpi_fpv_kinematics.inverse_kinematics import get_ik, set_link, get_link, set_joint_range, get_joint_range

from hiwonder_servo_msgs.msg import ServoStateList
from armpi_fpv_kinematics.msg import JointRange, JointsRange, Link
from armpi_fpv_kinematics.srv import SetRobotPose, SetJointValue, GetRobotPose, SetLink, GetLink, SetJointRange, GetJointRange
...
class SearchKinematicsSolutionsNode:
    def __init__(self, name):
        ...
        rospy.loginfo('kinematics init finish')
    ...
    def set_joint_range_srv(self, msg):
        ...
        set_joint_range([joint1.min, joint1.max], [joint2.min, joint2.max], [joint3.min, joint3.max],
                        [joint4.min, joint4.max], [joint5.min, joint5.max], 'deg')
        fk.set_joint_range([joint1.min, joint1.max], [joint2.min, joint2.max], [joint3.min, joint3.max],
                           [joint4.min, joint4.max], [joint5.min, joint5.max], 'deg')
    ...
    def get_servo_position(self, msg):
        ...
        for i in msg.servo_states:
            if 1 < i.id < 7:
                servo_positions.insert(0, i.position)
    ...
    def set_pose_target(self, msg):
        ...
        position, pitch, pitch_range, roll, resolution = msg.position, msg.pitch, msg.pitch_range, msg.roll, msg.resolution
        ...
        # t2 = rospy.get_time()
        # print(t2 - t1)
        # print(position, all_solutions, self.current_servo_positions)
        if len(all_solutions) and len(self.current_servo_positions):
            rpy = []
            min_d = 1000 * 5
            optimal_solution = []
            for s in all_solutions:
                pulse_solutions = transform.angle2pulse(s[0])
                # print(pulse_solutions, self.current_servo_positions)
                try:
